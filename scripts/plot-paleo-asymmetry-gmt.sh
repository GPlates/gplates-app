...
# plot paleo-asymmetry grid with GMT.
...
GRID_DIR="${DATA_DIR}/ReconstructedAsymmetryGrids"
OUTPUT_DIR="${GRID_DIR}/asymmetry-pmag-tif"
mkdir -p "${OUTPUT_DIR}"

while (( age <= max_age )); do
    echo "Plotting paleo-asymmetry for age ${age} Ma..."
    grid_file="${GRID_DIR}/pmag-ref-frame/Seton_etal_2020_Asymmetry_${age}Ma.nc"
    file_basename=asymmetry-platebounds-${age}Ma

    # if the files does not exist, they will not be plotted.
    # topology_boundaries=
    # subduction_left=
    # subduction_right=
    topology_boundaries="${DATA_DIR}/z22-topology/topology_${age}.00Ma.xy"
    subduction_left="${DATA_DIR}/z22-topology/topology_subduction_boundaries_sL_${age}.00Ma.xy"
    subduction_right="${DATA_DIR}/z22-topology/topology_subduction_boundaries_sR_${age}.00Ma.xy"
